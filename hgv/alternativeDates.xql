...
declare function local:getStrayAlternativeDates() as node()*
{
    for $doc in collection("/db/data/idp.data/dclp/HGV?select=*.xml;recurse=yes")
...
<html>
    <head>
        <title>HGV - Alternative Dates</title>
    ...
    <body>
        <h3>Alternativdatierungen</h3>
        { local:getStrayAlternativeDates() }
        { fn:current-dateTime() }
